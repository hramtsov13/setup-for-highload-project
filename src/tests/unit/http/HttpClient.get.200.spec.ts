...
describe('HttpClient.get', () => {
  before(() => {
    ...
    // Поскольку внутри HttpClient использует axios, имитируемы реализацию axios
    sinon.stub(axios, 'get').withArgs(mockParams.url, mockedRequestConfig).returns(mockedPromise);
  });
  after(() => {
    sinon.restore();
  });
